...
#include "inet/networklayer/ipv4/RlPpfpRoutingTable.h"
namespace inet
{
...
void Ipv4::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        directBroadcastInterfaceMatcher.setPattern(directBroadcastInterfaces.c_str(), false, true,
                                                   false);
...
void Ipv4::handleRegisterService(const Protocol &protocol, cGate *gate,
...
void Ipv4::handleRequest(Request *request)
{
    ...
    if (ctrl == nullptr)
        ...
        SocketDescriptor *descriptor = new SocketDescriptor(
            socketId, command->getProtocol() ? command->getProtocol()->getId() : -1,
        ...
        throw cRuntimeError("Unknown command: '%s' with %s", request->getName(),
                            ctrl->getClassName());
}
...
{
    if (msg->arrivedOn(
            "transportIn")) { // TODO packet->getArrivalGate()->getBaseId() == transportInGateBaseId
...
void Ipv4::handleIncomingDatagram(Packet *packet)
{
    ...
    if (packet->hasBitError()) {
        ...
        double relativeHeaderLength =
            B(ipv4Header->getHeaderLength()).get() / (double)B(ipv4Header->getChunkLength()).get();
    ...
    EV_DETAIL << "Received datagram `" << ipv4Header->getName()
...
void Ipv4::preroutingFinish(Packet *packet)
{
    ...
    if (fromIE->isLoopback()) {
        ...
        if (fromIE->getProtocolData<Ipv4InterfaceData>()->isMemberOfMulticastGroup(destAddr)
            ...
            EV_WARN << "Skip local delivery of multicast datagram (input interface not in "
                       "multicast group)\n";
        ...
        if (rt->isLocalAddress(destAddr)
            || fromIE->getProtocolData<Ipv4InterfaceData>()->getIPAddress().isUnspecified()) {
            reassembleAndDeliver(packet);
        } else if (destAddr.isLimitedBroadcastAddress()
                   || (broadcastIE = rt->findInterfaceByLocalBroadcastAddress(destAddr))) {
            ...
            if (broadcastIE && fromIE != broadcastIE && rt->isForwardingEnabled()) {
                if (directBroadcastInterfaceMatcher.matches(broadcastIE->getInterfaceName())
                    || directBroadcastInterfaceMatcher.matches(
                        broadcastIE->getInterfaceFullPath().c_str())) {
                    auto packetCopy = prepareForForwarding(packet->dup());
                    packetCopy->addTagIfAbsent<InterfaceReq>()->setInterfaceId(
                        broadcastIE->getInterfaceId());
                    packetCopy->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(
                        Ipv4Address::ALLONES_ADDRESS);
                    fragmentPostRouting(packetCopy);
                } else
                    EV_INFO << "Forwarding of direct broadcast packets is disabled on interface "
                            << broadcastIE->getInterfaceName() << std::endl;
            }
    ...
    if (ipv4Header->getDestAddress().isMulticast()) {
        ...
        if (rt->isLocalAddress(destAddr)) {
            if (destIE && !destIE->isLoopback()) {
                EV_DETAIL << "datagram destination address is local, ignoring destination "
                             "interface specified in the control info\n";
                destIE = nullptr;
            }
            if (!destIE)
...
Ipv4::determineOutgoingInterfaceForMulticastDatagram(const Ptr<const Ipv4Header> &ipv4Header,
                                                     const NetworkInterface *multicastIFOption)
{
    const NetworkInterface *ie = nullptr;
    if (multicastIFOption) {
        ie = multicastIFOption;
        EV_DETAIL << "multicast packet routed by socket option via output interface "
                  << ie->getInterfaceName() << "\n";
    }
    if (!ie) {
        ...
        if (ie)
            EV_DETAIL << "multicast packet routed by routing table via output interface "
                      << ie->getInterfaceName() << "\n";
    }
    if (!ie) {
        ie = rt->getInterfaceByAddress(ipv4Header->getSrcAddress());
        if (ie)
            EV_DETAIL << "multicast packet routed by source address via output interface "
                      << ie->getInterfaceName() << "\n";
    }
    if (!ie) {
        ie = ift->findFirstMulticastInterface();
        if (ie)
            EV_DETAIL << "multicast packet routed via the first multicast interface "
                      << ie->getInterfaceName() << "\n";
    }
    return ie;
}
...
void Ipv4::routeUnicastPacket(Packet *packet)
{
    ...
    if (destIE) {
        ...
        const char *pktName = packet->getFullName();
        if (regex_search(pktName, regex("probabilistic|multipath|singlepath|ppfp"))) {
            string modulePath = getParentModule()->getFullPath();
            pair<string, int> routePair;
            unordered_map<string, function<pair<string, int>(const string &, Packet *)>> routeMap =
                {
                    {"probabilistic",
                     [](const string &modulePath, Packet *packet) {
                         return RlProbabilisticRoutingTable::getInstance()->getRoute(modulePath,
                                                                                     packet);
                     }},
                    {"multipath",
                     [](const string &modulePath, Packet *packet) {
                         return RlMultipathRoutingTable::getInstance()->getRoute(modulePath,
                                                                                 packet);
                     }},
                    {"singlepath",
                     [](const string &modulePath, Packet *packet) {
                         return RlPathRoutingTable::getInstance()->getRoute(modulePath, packet);
                     }},
					{"ppfp", [](const string& path, Packet* packet) { return RlPpfpRoutingTable::getInstance()->getRoute(path, packet); }},
                };

            unordered_map<string, function<void(const string &, Packet *)>> countMap = {
                {"probabilistic",
                 [](const string &modulePath, Packet *packet) {
                     RlProbabilisticRoutingTable::getInstance()->countPktInNode(modulePath, packet);
                 }},
                {"multipath",
                 [](const string &modulePath, Packet *packet) {
                     RlMultipathRoutingTable::getInstance()->countPktInNode(modulePath, packet);
                 }},
                {"singlepath",
                 [](const string &modulePath, Packet *packet) {
                     RlPathRoutingTable::getInstance()->countPktInNode(modulePath, packet);
                 }},
				{"ppfp", [](const string& path, Packet* packet) { RlPpfpRoutingTable::getInstance()->countPktInNode(path, packet); }},
            };

            string pktNameStr(pktName);
            for (const auto& routeType : routeMap) {
                if (pktNameStr.find(routeType.first) != string::npos) {
                    routePair = routeType.second(modulePath, packet);
                    if (pktNameStr.find("distributed") != string::npos) {
                        countMap[routeType.first](modulePath, packet);
                    }
                    break;
                }
            }

            string s = routePair.first;
            NetworkInterface *ie = ift->getInterface(routePair.second);
            L3Address next;
            const char *da = s.c_str();
            cStringTokenizer tokenizer(da);
            const char *token = tokenizer.nextToken();
            L3AddressResolver().tryResolve(token, next);
            destIE = ie;
            packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(destIE->getInterfaceId());
            packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(next.toIpv4());
        }
        // conventional routing protocols
        else {
            const Ipv4Route *re = rt->findBestMatchingRoute(destAddr);
            if (re) {
                // statistical status information
                string srcDevice =
                    RlConventionalRoutingTable::getInstance()->getDeviceNameByAddress(
                        rt->getRouterId().str());
                string nextHopDevice =
                    RlConventionalRoutingTable::getInstance()->getDeviceNameByAddress(
                        re->getNextHopAsGeneric().str());
                if (srcDevice[0] == 'R' && nextHopDevice[0] == 'R') {
                    srcDevice.erase(srcDevice.begin(), srcDevice.begin() + 2);
                    nextHopDevice.erase(nextHopDevice.begin(), nextHopDevice.begin() + 2);
                    RlConventionalRoutingTable::getInstance()->countPkct(
                        atoi(srcDevice.c_str()), atoi(nextHopDevice.c_str()),
                        int(packet->getBitLength()));
                }
    ...
    if (!destIE) { // no route found
        ...
        emit(packetDroppedSignal, packet, &details);
        sendIcmpError(packet, fromIE ? fromIE->getInterfaceId() : -1, ICMP_DESTINATION_UNREACHABLE,
                      0);
...
{
    EV_INFO << "output interface = " << getDestInterface(packet)->getInterfaceName()
            << ", next hop address = " << getNextHop(packet) << "\n";
    numForwarded++;
    fragmentPostRouting(packet);
}
...
void Ipv4::routeLocalBroadcastPacket(Packet *packet)
{
    ...
    const NetworkInterface *destIE =
        interfaceReq != nullptr ? ift->getInterfaceById(interfaceReq->getInterfaceId()) : nullptr;
    ...
    if (destIE != nullptr) {
        packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(
            Ipv4Address::ALLONES_ADDRESS);
        fragmentPostRouting(packet);
        ...
        for (int i = 0; i < ift->getNumInterfaces(); i++) {
            ...
            if (!destAddr.isLimitedBroadcastAddress()) {
                Ipv4Address interfaceAddr =
                    ie->getProtocolData<Ipv4InterfaceData>()->getIPAddress();
                Ipv4Address broadcastAddr = interfaceAddr.makeBroadcastAddress(
                    ie->getProtocolData<Ipv4InterfaceData>()->getNetmask());
...
const NetworkInterface *
Ipv4::getShortestPathInterfaceToSource(const Ptr<const Ipv4Header> &ipv4Header) const
{
...
void Ipv4::forwardMulticastPacket(Packet *packet)
{
    ...
    // backward compatible: no parent means shortest path interface to source (RPB routing)
        ...
        // TODO no need to emit fromIE when tags will be used in place of control infos
        emit(ipv4DataOnRpfSignal, ipv4Header.get(),
             const_cast<NetworkInterface *>(fromIE)); // forwarding hook
        ...
        for (unsigned int i = 0; i < route->getNumOutInterfaces(); i++) {
            ...
            if (destIE != fromIE && outInterface->isEnabled()) {
                int ttlThreshold =
                    destIE->getProtocolData<Ipv4InterfaceData>()->getMulticastTtlThreshold();
                if (ipv4Header->getTimeToLive() <= ttlThreshold)
                    EV_WARN << "Not forwarding to " << destIE->getInterfaceName()
                            << " (ttl threshold reached)\n";
                else if (outInterface->isLeaf()
                         && !destIE->getProtocolData<Ipv4InterfaceData>()->hasMulticastListener(
                             destAddr))
                    EV_WARN << "Not forwarding to " << destIE->getInterfaceName()
                            << " (no listeners)\n";
                else {
        ...
        emit(ipv4MdataRegisterSignal, packet,
             const_cast<NetworkInterface *>(fromIE)); // postRouting hook
...
void Ipv4::reassembleAndDeliver(Packet *packet)
{
    ...
    if (ipv4Header->getSrcAddress().isUnspecified())
        EV_WARN << "Received datagram '" << packet->getName()
...
void Ipv4::reassembleAndDeliverFinish(Packet *packet)
{
    ...
    for (const auto &elem : socketIdToSocketDescriptor) {
        if ((elem.second->protocolId == -1 || elem.second->protocolId == protocol->getId())
            && (elem.second->localAddress.isUnspecified()
                || elem.second->localAddress == localAddress)
            && (elem.second->remoteAddress.isUnspecified()
                || elem.second->remoteAddress == remoteAddress)) {
            auto *packetCopy = packet->dup();
    ...
    if (contains(upperProtocols, protocol)) {
        ...
        EV_ERROR << "Transport protocol '" << protocol->getName()
                 << "' not connected, discarding packet\n";
        packet->setFrontOffset(ipv4HeaderPosition);
        // get source interface:
        const auto &tag = packet->findTag<InterfaceInd>();
        sendIcmpError(packet, tag ? tag->getInterfaceId() : -1, ICMP_DESTINATION_UNREACHABLE,
...
void Ipv4::decapsulate(Packet *packet)
{
    ...
    auto transportProtocol =
        ProtocolGroup::getIpProtocolGroup()->getProtocol(ipv4Header->getProtocolId());
...
{
    const NetworkInterface *destIE =
        ift->getInterfaceById(packet->getTag<InterfaceReq>()->getInterfaceId());
...
void Ipv4::fragmentAndSend(Packet *packet)
{
    ...
    // overTime
    if (simTime().dbl() - packet->getCreationTime().dbl() >= overTime) {
        delete packet;
        numDropped++;
        return;
    }
    ...
    if (ipv4Header->getDontFragment()) {
        PacketDropDetails details;
        emit(packetDroppedSignal, packet, &details);
        EV_WARN << "datagram larger than MTU and don't fragment bit set, sending "
    ...
    if (fragmentLength <= 0)
        throw cRuntimeError(
            "Cannot fragment datagram: MTU=%d too small for header size (%d bytes)", mtu,
            headerLength); // exception and not ICMP because this is likely a simulation configuration error, not something one wants to simulate
    ...
    while (offset < payloadLength) {
        ...
        if (lastFragment)
            curFragName += "-last";
        Packet *fragment =
            new Packet(curFragName.c_str()); // TODO add offset or index to fragment name
...
void Ipv4::encapsulate(Packet *transportPacket)
{
    ...
    ipv4Header->setProtocolId((IpProtocolId)ProtocolGroup::getIpProtocolGroup()->getProtocolNumber(
        transportPacket->getTag<PacketProtocolTag>()->getProtocol()));
    ...
    if (auto &dscpReq = transportPacket->removeTagIfPresent<DscpReq>()) {
        ...
        transportPacket->addTag<DscpInd>()->setDifferentiatedServicesCodePoint(
            ipv4Header->getDscp());
    }
...
void Ipv4::sendDatagramToOutput(Packet *packet)
{
    ...
    if (!ie->isBroadcast() || ie->getMacAddress().isUnspecified()) // we can't do ARP
        ...
        if (nextHopMacAddr.isUnspecified()) {
            ...
            ASSERT2(!containsKey(pendingPackets, nextHopAddr),
                    "Ipv4-ARP error: nextHopAddr found in ARP table, but Ipv4 queue for "
                    "nextHopAddr not empty");
...
void Ipv4::arpResolutionCompleted(IArp::Notification *entry)
{
    ...
    if (it != pendingPackets.end()) {
        ...
        EV << "ARP resolution completed for " << entry->l3Address << ". Sending "
           << packetQueue.getLength() << " waiting packets from the queue\n";
...
void Ipv4::arpResolutionTimedOut(IArp::Notification *entry)
{
    ...
    if (it != pendingPackets.end()) {
        cPacketQueue &packetQueue = it->second;
        EV << "ARP resolution failed for " << entry->l3Address << ",  dropping "
           << packetQueue.getLength() << " packets\n";
...
MacAddress Ipv4::resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr,
                                          const NetworkInterface *destIE)
{
    if (nextHopAddr.isLimitedBroadcastAddress()
        || nextHopAddr
               == destIE->getProtocolData<Ipv4InterfaceData>()->getNetworkBroadcastAddress()) {
...
void Ipv4::dropQueuedDatagram(const Packet *packet)
{
    ...
    for (auto iter = queuedDatagramsForHooks.begin(); iter != queuedDatagramsForHooks.end();
         iter++) {
    ...
    for (auto &elem : hooks) {
        ...
        switch (r) {
            ...
            case INetfilter::IHook::QUEUE:
                queuedDatagramsForHooks.push_back(
                    QueuedDatagramForHook(packet, INetfilter::IHook::PREROUTING));
                return r;
...
INetfilter::IHook::Result Ipv4::datagramForwardHook(Packet *packet)
{
    for (auto &elem : hooks) {
        ...
        switch (r) {
            ...
            case INetfilter::IHook::QUEUE:
                queuedDatagramsForHooks.push_back(
                    QueuedDatagramForHook(packet, INetfilter::IHook::FORWARD));
                return r;
...
INetfilter::IHook::Result Ipv4::datagramPostRoutingHook(Packet *packet)
{
    for (auto &elem : hooks) {
        ...
        switch (r) {
            ...
            case INetfilter::IHook::QUEUE:
                queuedDatagramsForHooks.push_back(
                    QueuedDatagramForHook(packet, INetfilter::IHook::POSTROUTING));
                return r;
        ...
        switch (r) {
            ...
            case INetfilter::IHook::QUEUE: {
                if (packet->getOwner() != this)
                    throw cRuntimeError(
                        "Model error: netfilter hook changed the owner of queued datagram '%s'",
                        packet->getFullName());
                queuedDatagramsForHooks.push_back(
                    QueuedDatagramForHook(packet, INetfilter::IHook::LOCALIN));
                return r;
            }
...
INetfilter::IHook::Result Ipv4::datagramLocalOutHook(Packet *packet)
{
    for (auto &elem : hooks) {
        ...
        switch (r) {
            ...
            case INetfilter::IHook::QUEUE:
                queuedDatagramsForHooks.push_back(
                    QueuedDatagramForHook(packet, INetfilter::IHook::LOCALOUT));
                return r;
