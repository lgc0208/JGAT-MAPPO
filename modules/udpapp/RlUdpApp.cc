 ...
 * @LastEditTime : 2025-08-10 13:45:35
 * @FilePath     : /root/lgc/RouterRL/modules/udpapp/RlUdpApp.cc
...
void RlUdpApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        unordered_map<string, function<void()>> initFunctions = {
			...
			{"ppfp", [&]() { RlPpfpRoutingTable::initTable(nodeNum, initRoutingTable, zmqPort, overTime, totalStep, returnModeId); }},
        };
        ...
        unordered_map<string, function<RlBasicRoutingTable *()>> getInstanceFunctions = {
			...
			{"ppfp", []() { return static_cast<RlBasicRoutingTable *>(RlPpfpRoutingTable::getInstance()); }},
        };
...
void RlUdpApp::sendPacket()
{
    ...
    if (senderNode == 0) {
        ...
        packet->addPar("stretch").setLongValue(0);
        sendPacketId++;
